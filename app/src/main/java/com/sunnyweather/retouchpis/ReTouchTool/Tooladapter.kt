...
import com.sunnyweather.retouchpis.MyApplication
import com.sunnyweather.retouchpis.R
import com.sunnyweather.retouchpis.ToolLogic

class Tooladapter(val toolList:List<Tool>,val retouchingphoto:ImageView,val retouchingView: View):RecyclerView.Adapter<Tooladapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        holder.itemView.setOnClickListener {
           ...
           if(lastClickPosition!=curClickPosition) //当前位置与上一次点击位置不一致
            {
                val toolLogic = ToolLogic(holder,retouchingphoto,0F,retouchingView)//获取实例
                ...
                val lastToolLogic = ToolLogic(lastHolder,retouchingphoto,lastProgress,retouchingView)//获取上一个的实例
